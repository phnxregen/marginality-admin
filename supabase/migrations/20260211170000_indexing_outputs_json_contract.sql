-- Persist final indexing JSON outputs (transcript + OCR) per video, linked to indexing_runs.
...
alter table public.indexing_outputs enable row level security;

drop policy if exists indexing_outputs_read_public_owner_or_assigned on public.indexing_outputs;
create policy indexing_outputs_read_public_owner_or_assigned
  on public.indexing_outputs
  for select
  using (
    exists (
      select 1
      from public.videos v
      where v.id = indexing_outputs.video_id
        and (
          v.visibility = 'public'
...
drop policy if exists indexing_outputs_service_all on public.indexing_outputs;
create policy indexing_outputs_service_all
  on public.indexing_outputs
  for all
